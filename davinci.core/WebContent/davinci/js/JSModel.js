...
davinci.js.JSElement.prototype.getSyntaxPositions = function(lineNumber){
	...
	function add(line,col,length,type)
	{
		if ((typeof lineNumber =="undefined") || lineNumber==line)
			positions.push({line:line, col:col, length:length, type:type});
	}
	
	function add2(pos,length,type)
	{
		if ((typeof lineNumber =="undefined")|| lineNumber==pos[0])
			positions.push({line:pos[0], col:pos[1], length:length, type:type});
	}
	var visitor={
...
davinci.js.JSFile.prototype.setText = function (text) {
	  var oldChildren=this.children;
	   ...
	   this.errors=[];
//		this.errors=davinci.js.JSElement.parse(text,this);
...
//	var positions =davinci.js.JSElement.prototype.getSyntaxPositions.apply(this,[lineNumber]);
//	
//
...
//		if ((typeof lineNumber =="undefined") || lineNumber==line)
//			positions.push({line:line,col:this.errors[i].character,length:this.errors[i].length, type:"error"});
//	}
//	
//		return positions;
//}
...
{ 
	...
	return s + this.left.getText(context)+this.operator+this.right.getText(context);
}
...
davinci.js.ConditionalExpression.prototype.getText = function (context) 
{
	...
	s+= this.condition.getText(context)+ " ? " + this.trueValue.getText(context) +" : "+  this.falseValue.getText(context);
	return s;
} 
...
davinci.js.Comment.prototype.addComment = function(type, startLne,startOffst,stopLne,stopOffst,text){
...
davinci.js.VariableFragment.prototype.visit = function(visitor){
  var dontVisitChildren;
	  ...
	  dontVisitChildren = visitor.visit(this);
  if (!dontVisitChildren){
	  if (this.init) this.init.visit(visitor);
  }
  if(visitor.endVisit) visitor.endVisit(this);
}
...
davinci.js.Block= function()
{
 ...
 davinci.js.Block.prototype.getText = function (context) {
	    ...
	    {
	    	s=s+this.printNewLine(context);
	    	s=s+this.children[i].getText(context)+(this.children[i].nosemicolon?"":";");
	    }
		context.indent-=2;
		s=s+this.printNewLine(context);
		s=s+"}"
   ...
   davinci.js.For= function()
   {
    ...
    davinci.js.ForIn= function()
    {
     ...
     if(this.label){
   	  ...
   	  s+="for ( "+this.iterationVar.getText(context)+" in "+this.collection.getText(context)+")";
 	  context.indent+=2;
   	  s=s+this.printStatement(context,this.action);
 	  context.indent-=2;
        return s;
     ...
     davinci.js.ForIn.prototype.getLabel = function(){
 	 	...
 	 	return 	  "for ( "+this.iterationVar.getLabel()+" in "+this.collection.getLabel()+")";
 	 }
        	  ...
        	  var s="";
         	  ...
         	  s+="do";
           	  context.indent+=2;
            	  s=s+this.printStatement(context,this.action);
           	  context.indent-=2;
         	  var s=s+this.printNewLine(context)+"while ( "+this.expr.getText(context)+" )";
                  return s;
