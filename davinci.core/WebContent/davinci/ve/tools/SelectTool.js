...
dojo.declare("davinci.ve.tools.SelectTool", davinci.ve.tools._Tool, {
    ...
    onDblClick: function(event){
        ...
        if (dojo.indexOf(selection, widget) >= 0) {
            if (ctrlKey && event.button !== 2) { // CTRL to toggle
                ...
                if (event.button !== 2) { // Right mouse not to alter selection
    ...
    _resetCursor: function(){
        if (!this._cursor) {
            ...
            //			dojo.style(this._feedback, "opacity", 0.1);
            this._timer = this._context.getGlobal().setInterval(function(node, context){
    ...
    onExtentChange: function(index, box, cursorOnly){
        ...
        if (!cursorOnly && ("w" in box || "h" in box)) {
            var resizable = davinci.ve.metadata.queryDescriptor(widget.type, "resizable"), w, h;
            ...
            if ("l" in box && "t" in box) {
                if (_node.style.position != "absolute") {
                    ...
                    if (close && close.widget && (close.widget != widget)) {
                        ...
                        var child = close.widget, parent = child.getParent(), index = parent.indexOf(child);
                    ...
                    if (!cursorOnly) {
                        var left = box.l, top = box.t, parentNode = widget.domNode.offsetParent;
                        ...
                        if (command) { // move and resize
                            ...
                            if (selection.length > 1) { // multiple move
                                var b = widget.getMarginBox(), dx = left - b.l, dy = top - b.t;
    ...
    _moveFocus: function(event){
        var direction = event.shiftKey ? -1 : +1, current = this._context.getSelection()[0], widgets = this._context.getTopWidgets(), nextIndex = current ? dojo.indexOf(widgets, current) + direction : (direction > 0 ? 0 : widgets.length - 1), next = widgets[nextIndex];
