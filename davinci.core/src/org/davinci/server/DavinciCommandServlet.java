...
public class DavinciCommandServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		CommandDescriptor commandDescriptor = (CommandDescriptor) this.commands
				.get(pathInfo);
	...
	private User checkLogin(HttpServletRequest req, HttpServletResponse resp,
			CommandDescriptor commandDescriptor) throws IOException {
		...
		if (user == null) {
			if (ServerManager.LOCAL_INSTALL) {
				...
				req.getSession().setAttribute(
						IDavinciServerConstants.SESSION_USER, user);
			} else {
	...
	void loadCommands() {
		...
		if (registry != null) {
			IExtensionPoint point = registry.getExtensionPoint(
					...
					IDavinciServerConstants.EXTENSION_POINT_COMMAND);
				...
				for (int i = 0; i < extensions.length; i++) {
					...
					for (int j = 0; j < elements.length; j++) {
						if (elements[j].getName().equals(
								IDavinciServerConstants.EP_TAG_COMMAND)) {
