...
public class Resource {
	...
	static {
		DirectoryFilter df = new DirectoryFilter(new String[] {
	...
	public static boolean isHidden(IVResource file) {
		for (int i = 0; i < Resource.resourceFilter.size(); i++) {
			IVResourceFilter filter = (IVResourceFilter) Resource.resourceFilter
					.get(i);
	...
	public static String vRsourcesToJson(IVResource listFiles[],
			boolean fullPath) {
	...
	public static String foundVRsourcesToJson(IVResource listFiles[]) {
		...
		for (int i = 0; i < listFiles.length; i++) {
			...
			jsonWriter.startObject().addField("file", name)
					.addFieldName("parents").startArray();
			for (int j = 0; j < parents.size(); j++) {
				if (Resource.isHidden((IVResource) parents.get(j))) {
					continue;
				}
				jsonWriter.startObject().addField("name",
						((IVResource) parents.get(j)).getName());
				jsonWriter.addFieldName("members").startArray();
				IVResource[] members = ((IVResource) parents.get(j))
						.listFiles();
				for (int k = 0; k < members.length; k++) {
					if (Resource.isHidden(members[k])) {
						continue;
					}
					jsonWriter.startObject().addField("isDir",
							members[k].isDirectory());
					...
					if (members[k] instanceof VLibraryResource) {
						...
						jsonWriter.addField("libraryId", r.getLibraryId());
						jsonWriter
								.addField("libVersion", r.getLibraryVersion());
					}
	...
	public static void directoryListJSON(IVResource dir, String path,
			User user, JSONWriter jsonWriter) {
		if (dir.exists()) {
			...
			for (int j = 0; j < listFiles.length; j++) {
				...
				jsonWriter.startObject().addField("name", member.getName())
						.addField("isDir", member.isDirectory()).endObject();
			}
			List links = user.getLinks().findLinks(path);
			for (Iterator iterator = links.iterator(); iterator.hasNext();) {
				Link link = (Link) iterator.next();
				jsonWriter.startObject().addField("name", link.name)
						.addField("isDir", true)
						.addField("link", link.location).endObject();
			}
		 ...
		 * ServerManager
		 * .getServerManger().getLibraryManager().listFiles(path,jsonWriter);
		 * 
		 * }
		 */
