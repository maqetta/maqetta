...
public class VLibraryResource implements IVResource {
	...
	public VLibraryResource(Library b, URL file, IVResource parent,
			String name, String bundleRoot) {
	...
	public String getPath() {
		String name = new Path(this.parent.getPath()).append(this.name)
				.toString();
	...
	public IVResource[] listFiles() {
		...
		for (int i = 0; i < files.length; i++) {
			...
			IPath newPath = itemPath.removeFirstSegments(myPath
					.matchingFirstSegments(itemPath));
			...
			item = new VLibraryResource(this.library, files[i], this, newPath
					.removeTrailingSeparator().toString(), new Path(
					this.bundleRoot).append(newPath).toString());
	...
	public IVResource get(String childName) {
		...
		for (int i = 0; i < files.length; i++) {
			...
			IPath newPath = itemPath.removeFirstSegments(myPath
					.matchingFirstSegments(itemPath));
			IVResource item = new VLibraryResource(this.library, files[i],
					this, newPath.removeTrailingSeparator().toString(),
					new Path(this.bundleRoot).append(newPath).toString());
