...
public class UserManagerImpl implements UserManager {
	...
	public UserManagerImpl() {
		...
		if (userDir == null) {
			userDir = (File) serverManger.servletConfig.getServletContext()
		...
		if (ServerManager.DEBUG_IO_TO_CONSOLE) {
			System.out.println("\nSetting [user space] to: "
					+ baseDirectory.getAbsolutePath());
		}
		System.out.println("\nSetting [user space] to: "
				+ baseDirectory.getAbsolutePath());

		String maxUsersStr = serverManger
				.getDavinciProperty(IDavinciServerConstants.MAX_USERS);
	...
	public User getUser(String userName) {
		...
		if (user == null && ServerManager.LOCAL_INSTALL
				&& IDavinciServerConstants.LOCAL_INSTALL_USER.equals(userName)) {
	...
	private void initializeNewUserFiles(File userDir) {
		List extensions = ServerManager.getServerManger().getExtensions(
				IDavinciServerConstants.EXTENSION_POINT_INITIAL_USER_FILES,
		...
		for (Iterator iterator = extensions.iterator(); iterator.hasNext();) {
			IConfigurationElement libraryElement = (IConfigurationElement) iterator
					.next();
			String path = libraryElement
					.getAttribute(IDavinciServerConstants.EP_ATTR_INITIAL_USER_FILE_PATH);
			String name = libraryElement.getDeclaringExtension()
	...
	public void copyDirectory(File userDir, String bundleDirName, Bundle bundle) {
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			try {
				...
				File destination = new File(userDir.getAbsolutePath() + "/"
						+ tail);
	...
	public User getSingleUser() {
		...
		userDir.mkdir();
		File settingsDir = new File(userDir,
	...
	private void createPersonManager() {
		IConfigurationElement libraryElement = ServerManager.getServerManger()
				.getExtension(
						IDavinciServerConstants.EXTENSION_POINT_PERSON_MANAGER,
						IDavinciServerConstants.EP_TAG_PERSON_MANAGER);
		if (libraryElement != null) {
			try {
				this.personManager = (PersonManager) libraryElement
						.createExecutableExtension(IDavinciServerConstants.EP_ATTR_PERSON_MANAGER_CLASS);
