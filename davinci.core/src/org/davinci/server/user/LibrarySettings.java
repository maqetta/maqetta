...
public class LibrarySettings extends XMLFile {
	...
	private LibInfo[] getDefaultLibs() {
		Library[] all = ServerManager.getServerManger().getLibraryManager()
				.getAllLibraries();
		LibInfo[] results = new LibInfo[all.length];
		for (int i = 0; i < all.length; i++) {
			results[i] = new LibInfo(all[i].getID(), all[i].getName(),
					all[i].getVersion(), all[i].getDefaultRoot());
		}
		return results;
	}
	...
	public boolean addLibrary(String name, String version, String id,
			String virtualRoot) {
	...
	public boolean removeLibrary(String id, String version) {
		for (int i = 0; i < this.libs.length; i++) {
			if (this.libs[i].getId().equals(id)
					&& this.libs[i].getVersion().equals(version)) {
	...
	protected Object createObject(Element element, String[] attributes) {
		...
		LibInfo link = new LibInfo(attributes[0], attributes[1], attributes[2],
				attributes[3]);
		return link;
	}
	...
	protected String[] getAttributeValues(Object object) {
		...
		return new String[] { link.getId(), link.getName(), link.getVersion(),
				link.getVirtualRoot() };
	}
