...
public class JSONReader {
	...
	private static Map parseObject(CharacterIterator it) {
		...
		while (true) {
			if (it.current() != '"') {
				throw JSONReader
						.error("expected a string start '\"' but was '" + it.current() + "'", it); //$NON-NLS-1$ //$NON-NLS-2$
			}
			...
			if (it.current() != ':') {
				throw JSONReader
						.error("expected a pair separator ':' but was '" + it.current() + "'", it); //$NON-NLS-1$ //$NON-NLS-2$
			}
			it.next();
			JSONReader.parseWhitespace(it);
			...
			if (it.current() != '}') {
				throw JSONReader
						.error("expected an object close '}' but was '" + it.current() + "'", it); //$NON-NLS-1$ //$NON-NLS-2$
			}
			break;
		}
		it.next();
	...
	private static List parseArray(CharacterIterator it) {
		...
		while (true) {
			...
			if (it.current() != ']') {
				throw JSONReader
						.error("expected an array close ']' but was '" + it.current() + "'", it); //$NON-NLS-1$ //$NON-NLS-2$
			}
			break;
		}
		it.next();
	...
	private static void parseText(String string, CharacterIterator it) {
		...
		for (int i = 0; i < length; i++) {
			if (c != string.charAt(i)) {
				throw JSONReader
						.error("expected to parse '" + string + "' but character " + (i + 1) + " was '" + c + "'", it); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$;
			}
			c = it.next();
		}
	}
	...
	private static Object parseNumber(CharacterIterator it) {
		...
		try {
			...
			throw JSONReader
					.error("expected a number but was '" + buffer.toString() + "'", it); //$NON-NLS-1$ //$NON-NLS-2$;
		}
	}
	...
	private static String parseString(CharacterIterator it) {
		...
		while (c != '"') {
			if (Character.isISOControl(c)) {
				throw JSONReader
						.error("illegal iso control character: '" + Integer.toHexString(c) + "'", it); //$NON-NLS-1$ //$NON-NLS-2$);
			}
			...
			if (c == '\\') {
				...
				case 'u':
					...
					try {
						...
						throw JSONReader
								.error("expected a unicode hex number but was '" + unicode.toString() + "'", it); //$NON-NLS-1$ //$NON-NLS-2$););
					}
					break;
				default:
					throw JSONReader.error(
