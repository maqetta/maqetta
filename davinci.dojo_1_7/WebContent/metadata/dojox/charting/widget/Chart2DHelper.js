dojo.provide("davinci.libraries.dojo.dojox.charting.widget.Chart2DHelper");

dojo.declare("davinci.libraries.dojo.dojox.charting.widget.Chart2DHelper", null, {
		...
		getChildrenData: function(/*Widget*/ widget, /*Object*/ options){
			...
			if(widget.dijitWidget.chart.series){
				dojo.forEach(widget.dijitWidget.chart.series, function(s){
					var seriesData = {type:"html.div", properties:{class:"series", name: s.name, data: s.data}};
					if(seriesData){
		...
		getPlotProperties: function(plot){
			...
			var metadata = davinci.ve.metadata.getMetadata(plot.declaredClass);
