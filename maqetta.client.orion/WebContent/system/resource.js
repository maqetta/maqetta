			...
			resourceChanged: function(type,changedResource){
				...
				if(changedResource == system.resource.getRoot()){
					...
					if(changedResource.parent){
						...
						var p1 = (new mPath(changedResource)).removeLastSegments();
...
//				console.log("parent:" + parent + " children :" + newChildrenList);
			...
			getWorkspace : function(){
				if(this.workspace==null){
					this.workspace = new system.model.Resource.Folder(".",null);
				}
...
//							resource.getChildren(function(){}, true);
						...
						for (var i=0;i<response.length;i++)
						{
							...
							for (var j=0;j<foundFile.parents.length;j++)
							{
								...
								if (j+1<foundFile.parents.length)
								{
									...
									if (!newResource) {
										newResource= new system.model.Resource.Folder(name,loadResource);
									}
	...
	dojo.subscribe("/davinci/resource/resourceChanged",system.resource, function(){return system.resource.resourceChanged;}());
});
