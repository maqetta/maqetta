...
],function(declare, entities, ContentPane, Button, RadioButton, SimpleTextarea, ResizeHandle, ViewLite, Metadata, veNls, commonNls){
	return declare("davinci.ve.widgets.PrimaryProperties", [ViewLite], {

		_getTemplate: function(){
//FIXME: Need to call this for each different widget
			var template = '';		
			var editBox = ''+
				'<div id="iedResizeDiv" class="iedResizeDiv" >' + 
				   '<textarea  dojoType="dijit.form.SimpleTextarea" name="davinciIleb" trim="true" class="';
			editBox += (this.multiLine === "true") ? 'smartInputTextAreaMulti' : 'smartInputTextArea';
			editBox += '"></textarea>'+
					'<div class="smartInputSim" ></div>'+
					'<div id="primaryResizeHandle" dojoType="dojox.layout.ResizeHandle" targetId="iedResizeDiv" constrainMin="true" maxWidth="200" maxHeight="600" minWidth="200" minHeight="55"  activeResize="true" intermediateChanges="true" ></div>' +
				'</div>';
			template += '<div class="primaryPropertiesContainer">' +
					editBox +
			'<div  id="davinci.ve.input.SmartInput_div"  class="davinciVeInputSmartInputDiv" >' + 
    			'<div id="davinci.ve.input.SmartInput_radio_div" class="smartInputRadioDiv" >' + 
    					...
    					'<tbody>' + 
    						'<tr> ' +
    							'<td class="smartInputTd1" > ' +
    								'<input id="primary_radio_text" showlabel="true" type="radio" dojoType="dijit.form.RadioButton" disabled="false" readOnly="false" intermediateChanges="false" checked="true"> </input> '+
		             			'</td> ' +
		             			'<td class="smartInputTd2" >'+ 
		             				'<div id="primary_radio_text_width_div" class="smartInputRadioTextDiv">'+
		             				'</div>'+
	             				'</td> ' +
             				'</tr>'+
             				'<tr> '+
             					'<td class="smartInputTd1"> <input id="primary_radio_html" showlabel="true" type="radio" dojoType="dijit.form.RadioButton"> </input>  </td> '+
             					'<td class="smartInputTd2">'+
             						'<div id="primary_radio_html_width_div" class="smartInputRadioTextDiv">'+
             						'</div>'+
	             				'</td> '+
         					'</tr> '+
     					'</tbody>'+ 
					...
					'<div class="smartInputHelpDiv" > '+
			        	...
			        	//'<button id="primary_ok"  dojoType="dijit.form.Button" type="submit" class="inlineEditHelpOk" >'+commonNls.buttonOk+'</button> <button id="primary_cancel" dojoType="dijit.form.Button" class="inlineEditHelpCancel"> '+commonNls.buttonCancel+'</button>  '+
			        	'</span>   '+
			        '</div> '+
					'<div id="davinci.ve.input.SmartInput_div_help" style="display:block;" class="smartInputHelpTextDiv" > '+
		...
		_getTextareaWidget: function(){
			if(!this.domNode){
				return null;
			}
			var textareaNode = this.domNode.querySelector('.smartInputTextArea');
			return textareaNode ? dijit.byNode(textareaNode) : null;
		},
		
		buildRendering: function(){
/*
//FIXME: Change so that each different widget type gets its own template
			var template = '<div class="primaryPropertiesContainer">';
			template += '<div class="primaryPropertiesTextarea"></div>';
			template += '<div>does this slide down?</div>';
			template += '</div>';
*/
			this.domNode =  dojo.doc.createElement("div");
/*
			this.domNode.innerHTML = template;
			var primaryPropertiesContainer = this.domNode.children[0];
			var textareaNode = this.domNode.querySelector('.primaryPropertiesTextarea');
			var textarea = new SimpleTextarea({'class':'primaryPropertiesTextarea', value:'def'}, textareaNode);
*/
		...
		onWidgetSelectionChange: function(){
			if(this._primaryPropertiesCP){
				this._primaryPropertiesCP.destroyRecursive();
				this._primaryPropertiesCP = null;
				this.domNode.innerHTML = '';
			}
			if(this._widget){
				var template = this._getTemplate();
				var text = this._widget._srcElement.getElementText(this._context); // just the inside text
				this._primaryPropertiesContainer = dojo.create('div', {'class':'primaryPropertiesContainer'}, this.domNode);
				this._primaryPropertiesCP = new ContentPane({}, this._primaryPropertiesContainer );
				this._primaryPropertiesCP.set('content', template);
				var textareaWidget = this._getTextareaWidget();
				textareaWidget.set('value', text);
				textareaWidget.set('disabled', false);
				Metadata.getSmartInput(this._widget.type).then(function(inline) {
					if(!inline){
						return;
					}
					this._inline = inline;
...
*/
			...
			var property = this._inline.property;
...
*/		
		_connectHelpDiv: function(){
			...
			this._connection.push(dojo.connect(dojo.byId('primary_ok'), "onclick", this, "onOk")); // same effect ad click away..
			this._connection.push(dojo.connect(dojo.byId('primary_cancel'), "onclick", this, "onCancel")); // same effect ad click away..
		},
/*		
	...
	;
			...
			this._inline._setStyleAttr({display: "block", top: top + 'px', left: left + 'px',  padding:"1px", overflow: "hidden", backgroundImage: "none"}); // padding needed to keep scroll bars off
			this._startTop = top;
...
*/		
		getFormat: function(){
			var htmlRadio = dijit.byId('primary_radio_html');
			var format = 'text';
...
//				return true;
		...
		setFormat: function(value){
			var htmlRadio = dijit.byId('primary_radio_html');
			var textRadio = dijit.byId('primary_radio_text');
...
//					}
		...
		updateFormats: function(){
			...
			var textObj = dojo.byId("primary_radio_text_width_div");
			...
			var htmlObj = dojo.byId("primary_radio_html_width_div");
			htmlObj.innerHTML = '<div id="primary_radio_html_div" class="dojoxEllipsis">'+veNls.htmlMarkup+'</div>';
			var htmlRadio = dijit.byId('primary_radio_html');
			var textRadio = dijit.byId('primary_radio_text');
...
/*		
		resize: function(e){
			...
			boxWidth = targetObj.clientWidth -8;
			...
			obj = dojo.byId("primary_radio_text_width_div");
			dojo.style(obj,'width',targetObj.clientWidth -50 + "px");
			obj = dojo.byId("primary_radio_html_width_div");
			dojo.style(obj,'width',targetObj.clientWidth -50 + "px");
		},
...
//				       '<input id="davinciIleb" class="davinciIleb smartInputTextBox" type="text"  dojoType="dijit.form.TextBox"  />' +
				   ...
				   '<div id="smartInputSim" class="smartInputSim" ></div>'+
					'<div id="primaryResizeHandle" dojoType="dojox.layout.ResizeHandle" targetId="iedResizeDiv" constrainMin="true" maxWidth="200" maxHeight="600" minWidth="200" minHeight="55"  activeResize="true" intermediateChanges="true" ></div>' +
		//			'<div id="primaryResizeHandle" dojoType="dojox.layout.ResizeHandle" targetId="iedResizeDiv" constrainMin="true" maxWidth="200" maxHeight="200" minWidth="200" minHeight="19" resizeAxis="x" activeResize="true" intermediateChanges="true" ></div>' +
				'</div>';
			if (this.multiLine === "true"){
				editBox = ''+
				'<div id="iedResizeDiv" class="iedResizeDiv" >' + 
...
//						'<div id="smartInputSim" style="height:10px; border-color: #B5BCC7; border-style: solid; border-width: 0px 3px 3px 3px;  background-color: #F7FCFF;"></div>'+
					'<div id="primaryResizeHandle" dojoType="dojox.layout.ResizeHandle" targetId="iedResizeDiv" constrainMin="true" maxWidth="200" maxHeight="600" minWidth="200" minHeight="80"  activeResize="true" intermediateChanges="true" ></div>' +
				'</div>';
			}
    					...
    					'<tbody>' + 
    						'<tr> ' +
    							'<td class="smartInputTd1" > ' +
		             			...
		             			'<td class="smartInputTd2" >'+ 
		             				'<div id="primary_radio_text_width_div" class="smartInputRadioTextDiv">'+
//			             				'<div id="primary_radio_text_div" class="dojoxEllipsis">'+
...
//			             				'</div>'+
             				...
             				'<tr> '+
             					...
             					'<td class="smartInputTd2">'+
             						'<div id="primary_radio_html_width_div" class="smartInputRadioTextDiv">'+
//	             						'<div id="primary_radio_html_div" class="dojoxEllipsis">'+
//	             							'HTML markup'+
//	             						'</div>'+
             						'</div>'+
	             				'</td> '+
		...
		_connectResizeHandle: function(){
			var resizeHandle = dijit.byId('primaryResizeHandle');
